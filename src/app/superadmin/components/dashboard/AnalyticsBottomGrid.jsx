    ...
    return (
        <div className="grid grid-cols-2 gap-3">
            {delivery.map((item, idx) => (
                <div key={idx} className="bg-white p-3 rounded-[20px] border border-[#E2E8F0] shadow-sm flex items-center gap-4 hover:shadow-md transition-all cursor-default group">
                    <div className={`w-16 h-16 rounded-2xl ${item.bg} flex items-center justify-center ${item.color} group-hover:scale-105 transition-transform`}>
                        <Icon icon={item.icon} width="26" />
                    </div>
                    <div className="space-y-0.5">
                        <p className="text-[13px] font-medium text-[#64748B] tracking-tight">{item.label}</p>
                        <h5 className="text-[20px] font-extrabold text-[#0F172A] leading-tight tracking-tight">{item.value}</h5>
                    </div>
                </div>
            ))}
        </div>
    ...
    return (
        <div className="grid grid-cols-2 gap-3">
            {marketing.map((item, idx) => (
                <div key={idx} className="bg-white p-4 rounded-[20px] border border-[#E2E8F0] shadow-sm flex flex-col justify-center min-h-[85px] hover:shadow-md transition-all cursor-default">
                    <p className="text-[11px] font-semibold text-[#94A3B8] uppercase tracking-wide mb-1">{item.label}</p>
                    <h5 className="text-[20px] font-extrabold text-[#0F172A] tracking-tight leading-none">{item.value}</h5>
                </div>
            ))}
        </div>
    );
        ...
        <div className="bg-white rounded-lg shadow p-4 h-full">
            ...
            <div className="space-y-2 overflow-y-auto pt-3 flex-1 hide-scrollbar border-t border-[#E2E8F0]">
                {activities.map((activity, idx) => (
                    <div
                        key={idx}
                        className={`${activity.bgColor} ${activity.borderColor} border rounded-lg p-2 transition-all hover:shadow-sm cursor-default`}
                    >
                        ...
                        <p className={`text-[11px] font-medium ${activity.descColor}  mt-2 mb-1.5`}>
                            {activity.desc}
                        </p>
...
const AnalyticsBottomGrid = () => {
    return (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[0.9fr_1fr_1fr] gap-4 font-manrope pt-2">
            {/* Delivery & Marketing Column */}
            <div className="lg:col-span-0.5 space-y-10 pt-4">
                {/* Delivery Section */}
