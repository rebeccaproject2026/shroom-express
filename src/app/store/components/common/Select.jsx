...
const Select = ({
    ...
    const renderOptionContent = (item, index) => {
        ...
        if (showProductInfo || showAvatar) {
            return (
                <div
                    className={`group flex items-start gap-3 px-2 py-1 cursor-pointer border-b border-[#E8E8E8]  last:border-0 transition
        ${isHighlighted
    ...
    return (
        ...
        >
            <button
                ...
                className={`head-dr-dropdown form-select w-full px-4.5 text-[13px] border border-[#E8E8E8] rounded-full font-medium focus:outline-none shadow-none flex items-center justify-between pr-8 transition-colors text-[#22222] placeholder-[#222222] ${compact ? "min-h-[32px] py-2" : "min-h-[40px] py-2.5"
                    } ${disabled
                        ? "bg-gray-100 text-gray-400 cursor-not-allowed border-gray-200"
                        : "bg-white text-gray-900 cursor-pointer"
                    } ${className}`}
            >
                <span className="overflow-hidden whitespace-nowrap text-sm flex-1 text-left text-gray-[#222222] font-semibold">
                    {displayOption || displayTitle}
                </span>
                <span className={`absolute right-5 top-1/2 -translate-y-1/2  flex items-center justify-center ${disabled ? "text-gray-300" : "text-gray-500"}`}>
                    {isOpen ? (
                        <Icon icon="lineicons:chevron-up" width={20} height={20} color="black" />
                    ) : (
                        <Icon icon="lineicons:chevron-down" width={20} height={20} color="black" />
                    )}
                </span>
            </button>
                    ...
                    {showSearch && (
                        <div className="p-2 border-b border-gray-200 shrink-0">
                            ...
                            <input
                                ...
                                className="w-full px-2.5 py-1.5 text-[13px] border border-[#000] rounded-sm bg-white text-gray-700 placeholder-[#222222]focus:outline-none "
                            />
                        </div>
                    )}
