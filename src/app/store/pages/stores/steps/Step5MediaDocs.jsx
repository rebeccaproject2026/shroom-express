...
const UploadBox = ({ label, sub, icon, isLogo, onClick }) => (
  <div
    onClick={onClick}
    className={`w-full aspect-[6/1.2] border-2 border-dashed rounded-md flex flex-col items-center justify-center gap-2 bg-[#F8F8F8] cursor-pointer transition-all group overflow-hidden relative ${isLogo ? 'border-[#E93E2B]' : 'border-[#BDBDD2]'}`}
  >
    <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${isLogo ? 'bg-[#FFF0EE] text-[#E93E2B]' : 'bg-[#F1F5F9] text-[#64748B]'}`}>
      ...
      <div className="p-4 border-b border-[#BDBDD2] flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-[#FFEDEB] rounded-sm flex items-center justify-center shrink-0">
            <Icon icon="solar:folder-outline" className="text-[#E93E2B]" width="24" />
          </div>
          <div>
      ...
      <div className="p-5 space-y-6">
        ...
        <div className="space-y-4">
          <label className="text-sm font-semibold text-[#181211] block">
            Store Logo <span className="text-[#E93E2B] ml-0.5">*</span>
          </label>
          <UploadBox
        ...
        <div className="space-y-4">
          <label className="text-sm font-semibold text-[#181211] block">
            Store Banner Image <span className="text-[#E93E2B] ml-0.5">*</span>
          </label>
          <UploadBox
        ...
        {(formData.logo || formData.banner) && (
          <div className="relative rounded-md overflow-visible h-[140px] bg-cover bg-center border border-[#BDBDD2] mb-2 animate-in fade-in slide-in-from-top-2 duration-300"
            ...
            <div className="relative h-full flex items-end justify-between px-6 pb-3">
              <div className="absolute -bottom-8 left-6 w-24 h-24 rounded-full flex items-center justify-center overflow-hidden z-20 ring-4 ring-white shadow-lg bg-white">
                {formData.logo ? (
                  <img src={formData.logo} alt="Logo" className="w-full h-full object-cover" />
                ) : (
                  <div className="w-full h-full bg-[#F8FAFC] flex items-center justify-center">
                    <Icon icon="lucide:store" className="text-[#E93E2B]" width="32" />
                  </div>
                )}
              </div>
              <div className="ml-28 flex-1 min-w-0 pb-1">
                ...
                <div className="flex items-center gap-1 text-[11px] font-bold">
                  <span className="text-[#E93E2B] hover:underline cursor-pointer">Home</span>
                  <span className={formData.banner ? 'text-white' : 'text-[#181211]'}>/</span>
                  <span className="text-[#E93E2B] hover:underline cursor-pointer">Stores</span>
                  <span className={formData.banner ? 'text-white' : 'text-[#181211]'}>/</span>
                  <span className={`truncate ${formData.banner ? 'text-white' : 'text-[#181211]'}`}>{storeName}</span>
                </div>
              </div>
            </div>
          </div>
